...
const FinishLine = () => {
  ...
  const handlePendingSave = (pendingItem) => {
    ...
    if (riderOnTrack) {
      ...
      toast.error(
        `Rider #${pendingItem.riderNumber} is not on track or already finished!`,
      );
    }
  };
      ...
      <Card className="overflow-hidden">
        ...
        <div className="flex bg-slate-50 border-b border-slate-200">
          <TabButton
            ...
            icon={Flag}
            count={
              pendingFinishes.length > 0 ? pendingFinishes.length : undefined
            }
          ...
          <TabButton
            label="On Track"
            icon={List}
            count={ridersOnTrack.length}
            onClick={() => setActiveTab("onTrack")}
            isActive={activeTab === "onTrack"}
            color="red"
          />
          <TabButton
            label="Log"
            ...
            <div className="space-y-4 animate-in fade-in">
              ...
              <div className="space-y-2 max-h-80 overflow-y-auto">
                {pendingFinishes.map((item) => (
                  ...
                  >
                    <div className="bg-slate-800 text-white font-mono rounded px-2 py-1 text-sm font-bold">
                      {item.finishTime}
                    </div>
                    <button
                      onClick={() => {
                    ...
                    >
                      {item.riderNumber || (
                        <span className="text-slate-300">Bib #</span>
                      )}
                    </button>
                    <button
                      onClick={() => handlePendingSave(item)}
                      className="p-2 rounded text-white bg-green-500 hover:bg-green-600 disabled:bg-slate-300"
                    ...
                    <button
                      ...
                      className="p-2 rounded bg-slate-200 text-slate-500 hover:bg-slate-300"
                    >
                      <Trash2 size={16} />
                    </button>
                  </div>
            ...
            <div className="space-y-3 animate-in fade-in">
              <div className="bg-slate-100 rounded-lg p-2 flex justify-between items-center text-xs">
                ...
                <button
                  ...
                  className={`font-bold px-3 py-1 rounded-md transition-colors ${soloMode ? "bg-blue-600 text-white" : "bg-white"}`}
                >
                  {soloMode ? "ON" : "OFF"}
                </button>
              </div>
              ...
              {soloMode && (
                ...
                >
                  <input
                    ...
                    className="flex-1 text-lg font-bold p-2 text-center border border-slate-300 rounded-lg"
                    placeholder="Start Bib #"
                    autoFocus
                  />
                  <button
                    type="submit"
                    disabled={!soloNumber}
                    className="bg-green-600 hover:bg-green-700 text-white font-bold px-4 rounded-lg disabled:bg-slate-300"
                  >
                    START
                  </button>
                </form>
                ...
                {ridersOnTrack.map((rider) => (
                  ...
                  >
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => handleDirectFinish(rider)}
                        disabled={finishing === rider.riderNumber}
                        className="bg-red-600 hover:bg-red-700 text-white font-bold px-5 py-2 rounded-lg text-sm w-24 text-center"
                      >
                      ...
                      <button
                        onClick={() =>
                          confirmDialog.current.open({
                            title: "Confirm DNF",
                            message: `Mark rider #${rider.riderNumber} as DNF?`,
                            onConfirm: () => updateRiderStatus(rider.id, "DNF"),
                          })
                        }
                        className="bg-slate-400 hover:bg-slate-500 text-white font-bold px-3 py-2 rounded-lg text-xs"
                      >
                        DNF
                      </button>
                    </div>
              ...
              <div>
                ...
                <div className="bg-slate-50 rounded-lg border border-slate-200">
                  {finishedRiders.slice(0, 5).map((rider) => (
                    <div
                      key={rider.id}
                      className="p-2 border-b border-slate-200 last:border-0 flex justify-between items-center"
                    >
              ...
              <div>
                ...
                <div className="bg-slate-50 rounded-lg border border-slate-200">
                  {finishLogs.slice(0, 10).map((log, i) => (
                    <div
                      key={i}
                      className="p-2 border-b border-slate-200 last:border-0 flex justify-between items-center text-xs"
                    >
